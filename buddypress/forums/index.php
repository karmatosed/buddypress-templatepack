		...
		<?php if ( is_user_logged_in() ) : ?>
			...
			<?php if ( bp_is_active( 'groups' ) && bp_has_groups( 'user_id=' . bp_loggedin_user_id() . '&type=alphabetical&max=100&per_page=100' ) ) : ?>
				...
				<form action="" method="post" id="form-topic-form" class="form-buddypress">
					...
					<!-- really we should change to use all _ or - but this will break tons-->
					<label><?php _e( 'Title:', 'buddypress' ); ?></label>
					<input type="text" name="topic_title" id="topic-title" value="" maxlength="100" />

					<label><?php _e( 'Content:', 'buddypress' ); ?></label>
					<textarea name="topic_text" id="topic-text"></textarea>

					<label><?php _e( 'Tags (comma separated):', 'buddypress' ); ?></label>
					<input type="text" name="topic_tags" id="topic-tags" value="" />

					<label><?php _e( 'Post In Group Forum:', 'buddypress' ); ?></label>
					<select id="topic-group-id" name="topic_group_id">
